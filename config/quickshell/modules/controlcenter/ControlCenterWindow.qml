...
PanelWindow {
    ...
    Process {
        id: airmonStartProcess
        command: ["sudo", "airmon-ng", "start", "wlan0"]
        onExited: (code, status) => root.monitorActive = true
    }

    Process {
        id: airmonStopProcess
        command: ["sudo", "airmon-ng", "stop", "wlan0mon"]
        onExited: (code, status) => root.monitorActive = false
    }
    ...
    implicitWidth: 430
    implicitHeight: Math.min(780, screen.height - 40) //750
    color: "transparent"
                ...
                Flickable {
                    ...
                    layer.effect: OpacityMask {
                        maskSource: Rectangle {
                            ...
                            gradient: Gradient {
                                ...
                                GradientStop { position: 0.0;  color: Qt.rgba(1, 1, 1, Math.max(0, 1 - contentFlick.contentY / 40)) }
                                GradientStop { position: 0.06; color: "#ffffffff" }
                                GradientStop { position: 0.90; color: "#ffffffff" }
                    ...
                    ColumnLayout {
                        ...
                        GridLayout {
                            ...
                            QuickToggle {
                                Layout.fillWidth: true
                                icon: root.monitorActive ? "󱑽" : "󰖩"
                                label: "Monitor"
                                subLabel: root.monitorActive ? "wlan0mon active" : "wlan0 managed"
                                active: root.monitorActive
                                activeColor: root.cPrimary
                                surfaceColor: root.cSurfaceContainerHigh
                                textColor: root.cOnSurface
                                onClicked: {
                                    if (root.monitorActive)
                                        airmonStopProcess.running = true
                                    else
                                        airmonStartProcess.running = true
                                }
                            }

                            QuickToggle {
                                Layout.fillWidth: true
                                icon: root.idleInhibitor.inhibited ? "󰒳" : "󰒲"
                                label: "Caffeine"
                                subLabel: root.idleInhibitor.inhibited ? "Screen always on" : "Auto sleep on"
                                active: root.idleInhibitor.inhibited
                                activeColor: root.cPrimary
                                surfaceColor: root.cSurfaceContainerHigh
                                textColor: root.cOnSurface
