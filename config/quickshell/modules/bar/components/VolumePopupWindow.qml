...
PanelWindow {
    ...
    onShouldShowChanged: {
        if (shouldShow) mouseHasEntered = false
    }
    readonly property var pywal: QsServices.Pywal
    readonly property var audio: QsServices.Audio
    
    // Material 3 colors
    readonly property color m3Surface: Qt.rgba(pywal.background.r, pywal.background.g, pywal.background.b, 0.18)
    ...
    HoverHandler {
        onHoveredChanged: {
            if (hovered) {
                ...
                if (popupWindow.mouseHasEntered)
                    closeTimer.restart()
            }
        }
    }
    ...
    Timer {
        id: closeTimer
        interval: 100
        onTriggered: popupWindow.shouldShow = false
    }
